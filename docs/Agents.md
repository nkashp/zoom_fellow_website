...
ZOOM_REDIRECT_URL=https://agilely-gabelled-clementine.ngrok-free.dev/api/auth/callback
...
### Deployment
The app runs on EC2 with a static ngrok tunnel (`agilely-gabelled-clementine.ngrok-free.dev`) for development. The Zoom Marketplace app is configured with OAuth redirect URL and RTMS webhook URL pointing to the ngrok domain.
